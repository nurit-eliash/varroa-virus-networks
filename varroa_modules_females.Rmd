...
---
this analysis starts with the 66 libraries (i removed the 5 outlirered libraries from the "data/kallisto" folder in GitHub, however they still exsist in the local folder: "/Users/nuriteliash/OneDrive - OIST/Repos/varroa-virus-networks-Local/data/kallisto.rds".
...
# Load all 71 libraries with all varroa genes and viruses
...
# and 9 non-female libraries (SRR5760848, SRR5760838, SRR5760828, SRR5760818, SRR5377265 ,SRR5377270 ,SRR5377269 ,SRR5377263 ,SRR5377266)
...
the aim of analyzing only female modules is to find matching modules between the 66 lib (containing all mites), and the 57 libs (excluding male libs).
as we wish to generalize our anlaysis conclusions to all mites, we aim to detect specific genes that are important in both types of analysis (66 libs and 57 libs)
for that, we will construct networks from 57 libs, aiming to have about the same number of modules, in both WGCNAs. 
playing with the "mergeCutHeight" value, i find that "mergeCutHeight = 0.22" gives 14 modules, the same as in the 66 libs, with "mergeCutHeight = 0.25". 
...
lnames <- load(file = "/Users/nuriteliash/OneDrive - OIST/Repos/varroa-virus-networks-Local/results/varroa_virus-01-dataInput_57.RData"); 

#The variable lnames contains the names of loaded variables. 
lnames 

# Load network data saved in the second part.
lnames = load(file = "/Users/nuriteliash/OneDrive - OIST/Repos/varroa-virus-networks-Local/results/Varroa_modules_networkConstruction-auto_57.RData"); 
lnames
...
moduleTraitCor = cor(MEs_57, b_viruses_load_57, use = "p");
...
moduleTraitCor_57 <- as.matrix(moduleTraitCor)
moduleTraitPadjust_57 <- as.matrix(moduleTraitPadjust)
...
names <- load(file = "/Users/nuriteliash/OneDrive - OIST/Repos/varroa-virus-networks-Local/results/moduleTraitCor_57.RData"); 
names

# and the viral abundance correlogram
names <- load(file = "/Users/nuriteliash/OneDrive - OIST/Repos/varroa-virus-networks-Local/results/virusAbundCor_57.RData"); 
names
...
MMPvalue = as.data.frame(corPvalueStudent(as.matrix(geneModuleMembership_57), nSamples));
...
GSPvalue = as.data.frame(corPvalueStudent(as.matrix(geneTraitSignificance_57), nSamples));
...
names(GSPvalue) = paste("p.GS.", names(b_viruses_load_57), sep="");

#save data:
save(geneTraitSignificance_57,geneModuleMembership_57, file = "/Users/nuriteliash/OneDrive - OIST/Repos/varroa-virus-networks-Local/results/geneTraitANDgeneMM_57.RData")
```
