...
The following chunks are ordered as the sections in the Materials and Methods of the manuscript "Diverse and rapidly evolving viral strategies modulate arthropod vector gene expression", [Eliash et al. (2022)](https://www.biorxiv.org/content/10.1101/2021.12.16.472853v1). All of the referred tables and figures can be found in the manuscript as well. 
...
### Detect outlier libraries based on PCA (Additional file 9)
...
# for MS, export as pdf, 12 x 10 cm, portrait
```

### Figure 3a. Virus correlation matrix
the viral correlation analysis includes 66 libraries, and 17 viruses (viruses that appear in at least one of the libraries)
```{r viral correlation, fig.cap = "Figure 3a. Correlation between viruses’ loads. P-values of the Pearson coefficient are adjusted according to FDR-correction; correlation significance marked by (*) 0.1 < Padjust < 0.01; (**) Padjust < 0.01. All viruses’ loads are transformed by log10 of the (TPM), Transcripts Per Million." , message=FALSE, warning=FALSE}

load(file = "/Users/nuriteliash/Documents/GitHub/varroa-virus-networks/results/viruses_load.rds")

# prepare the 'viruses_load' table, with 17 viruses, out of the initial 25
# excluding viruses with 'zero tpm': "CBPV", "AFV", "ANV" , "VPVL_46", "VPVL_36", "SBPV", also excluding viruses with low abundance :"KBV", "LSV".  left with 17 viruses
viruses_load_17 <- viruses_load %>%
  filter(description %in% c("DWVa","ARV-2","ARV-1","VOV_1","BMV","AmFV","ABPV","VTLV","IAPV","SBV","DWVb","DWVc","BQCV","VDV3","VDV2","VDV4","VDV5")) %>%
...
corrplot2 <- function(data,
  ...
  corrplot(mat,
    ...
    tl.col = "black", 
    diag = diag,
p.mat = p.mat,  #  to know which of the correlations is significant
sig.level = sig.level)
}
...
# and the table of viral load of the 17 viruses in 66 libraries:
#save(viruses_load_17, file = "results/viruses_load_17.rds")
```

```{r eval=FALSE, include=FALSE}
# for the paper figure 3a
# make a function to correlate viruses load
corrplot2 <- function(data,
                      method = "pearson",
                      sig.level = 0.05,
...
corrplot2(
  ...
  sig.level = 0.05,
  order = "original",
  diag = TRUE, # add a diagonal so it will fit the other matrix fig3b, in figure 3
  type = "lower" # arrange in "lower" instead of "upper" type
  )

# export as pdf 8 x 9 inches
```

## Gene network analysis
We used a network analysis approach to identify groups of genes that share a similar expression pattern across a large set of available varroa transcriptomic data (RNAseq). To construct the gene-network, a weighted gene co-expression analysis was carried out using the WGCNA package in R, following the authors’ tutorial ([Langfelder and Horvath 2016](https://horvath.genetics.ucla.edu/html/CoexpressionNetwork/Rpackages/WGCNA/Tutorials/); [Langfelder and Horvath 2008](https://bmcbioinformatics.biomedcentral.com/articles/10.1186/1471-2105-9-559)). 
...
A gene co-expression network is a a systems-biology approach, based on the assumption that biological functions are carried out in a modular manner ([Barabási and Oltvai 2004](http://www.cs.cmu.edu/~epxing/Class/10810-06/readings/Barabasi_Oltvai_04.pdf)). We identify co-expressed genes that have similar expression patterns across samples using hierarchical clustering and place them in gene groups, called ‘modules’ ([Stuart et al. 2003](https://www.science.org/doi/10.1126/science.1087447)). 

### (1) Network construction and module detection
Following the steps in WGCNA tutorial: "Automatic, one-step network construction and module detection" ([Langfelder and Horvath 2016](https://horvath.genetics.ucla.edu/html/CoexpressionNetwork/Rpackages/WGCNA/Tutorials/))

#### Choosing the soft-thresholding power: analysis of network topology (Additional file 2)
...
```{r sft plot,fig.cap = "Additional file 2a.  Network construction using 10,247 genes of 66 SRA varroa libraries.  Picking soft threshold.", message=FALSE, warning=FALSE,fig.show="hold", out.width="50%"}
# Plot the results:
sft_df <- data.frame(
...
#### Constructing the gene network and identifying modules is now a simple function call: (Additional file 2b)
...
```{r Hierarchical clustering dendrogram, fig.cap = "Additional file 2b. Network construction using 10,247 genes of 66 SRA varroa libraries. Hierarchical clustering dendrogram using merge cut height of 25%, revealing 12 co-expressed genes modules. Each branch of the dendrogram represents a single gene, and the colored bar below denotes its corresponding module, as annotated in the legend to the right. The dendrogram height is the distance between genes.", message=FALSE, warning=FALSE}
...
### (2) Figure 3b. Correlating modules to viral load
To test if the varroa modules interact with the different viruses it carries, we correlated the module eigengenes to the viruses’ load (log10TPM). We used Pearson correlation method and adjusted the p-values for multiple comparisons using the Benjamini–Hochberg method to control the false discovery rate (Benjamini and Hochberg 1995) (Fig 3b). 
```{r Quantifying module–trait associations,  message=FALSE, warning=FALSE}
# For correlating varroa modules to viral load, we gonna use the table of 17 viruses saved in chunk "viral correlation", in section "viral correlation matrix", and correlated these to the varroa modules eigengenes found in the previous chunk. 
...
# correlate the modules eigengenes (MEs) with the viral load (viruses_load_17)
moduleTraitCor_66 = cor(MEs, viruses_load_17, use = "p") %>%
  as.matrix()
...
# using p.adjust function, for all comparisons, 12 modules and 17 viruses (m=204). 
...
```{r eval=FALSE, include=FALSE}
# figure 3b for the paper (without values within the heatmap)
par(mar = c(6, 8.5, 3, 3));
labeledHeatmap(Matrix = moduleTraitCor_66,
               xLabels = names(viruses_load_17),
               yLabels = names(MEs),
               ...
               colors = hcl.colors(n = 50, palette = "RdBu", alpha = NULL, rev = FALSE, fixup = TRUE),
               invertColors = TRUE,
               #textMatrix = textMatrix, # dont include text
               setStdMargins = FALSE,
               cex.text = 0.7,
               zlim = c(-1,1),
               main = paste("Varroa Module-viruses relationships"))
# save as pdf, 8 x 9 inches
```

Correlating varroa modules eigengenes to their viruses’ loads (transcript per million, TPM), we found significant interactions between specific modules and viruses (Pearson correlation followed by Benjamini-Hochberg FDR-correction, P-adjust < 0.1, Fig 3b). 
...
# virusNames = substring(names(viruses_load_17), 1)
...
##### Identifying genes with high Module-membership in module 10 (the purple module)
...
# this part is not included in the html output
# make a table with the Trait-significance ("GS") for each gene (which is its correlation coefficient, pearson)
geneTraitSignificance_66 = as.data.frame(cor(for_modules, viruses_load_17, use = "p"));
...
names(geneTraitSignificance_66) = paste("GS.", names(viruses_load_17), sep="");
names(GSPadjust_66) = paste("pdj.GS.", names(viruses_load_17), sep="");
```
...
### (4) GO-term enrichment analysis for varroa modules (Additional file 3)

Analysis based on [Tehcer et al. (2019)](https://www.nature.com/articles/s42003-019-0606-0)
...
# change "tan" to the name of the desired module, in the first line: [moduleColors=="tan"], and in the final "write.csv(file = "GO_term_enrichment_**tan**BP.csv")
...
All GO-terms of the significantly interacting modules are also available as csv files, on the [github project result folder](https://github.com/nurit-eliash/varroa-virus-networks/tree/master/results) 

#### Significant GO-terms of the interacting modules {.tabset}

##### Module 1
```{r GO-terms module number 1, message=FALSE, warning=FALSE}
GO_term_turquoise <- read.csv("/Users/nuriteliash/Documents/GitHub/varroa-virus-networks/results/GO_terms_modules/GO_term_turquoise.csv")
GO_term_turquoise %>%
  kbl() %>%
...
```
##### Module 2
```{r GO-terms module number 3, message=FALSE, warning=FALSE}
GO_term_blue <- read.csv("/Users/nuriteliash/Documents/GitHub/varroa-virus-networks/results/GO_terms_modules/GO_term_blue.csv")
GO_term_blue %>%
  kbl() %>%
  kable_styling(bootstrap_options = "striped", full_width = F, position = "left") %>%
  kable_paper() %>%
...
##### Module 3
```{r GO-terms module number 3, message=FALSE, warning=FALSE}
GO_term_brown <- read.csv("/Users/nuriteliash/Documents/GitHub/varroa-virus-networks/results/GO_terms_modules/GO_term_brown.csv")
GO_term_brown %>%
  kbl() %>%
  kable_styling(bootstrap_options = "striped", full_width = F, position = "left") %>%
  kable_paper() %>%
...
```
##### Module 4
```{r GO-terms module number 5, message=FALSE, warning=FALSE}
GO_term_yellow <- read.csv("/Users/nuriteliash/Documents/GitHub/varroa-virus-networks/results/GO_terms_modules/GO_term_yellow.csv")
GO_term_yellow %>%
  kbl() %>%
  kable_styling(bootstrap_options = "striped", full_width = F, position = "left") %>%
  kable_paper() %>%
...
```
##### Module 6
```{r GO-terms module number 6, message=FALSE, warning=FALSE}
GO_term_red <- read.csv("/Users/nuriteliash/Documents/GitHub/varroa-virus-networks/results/GO_terms_modules/GO_term_red.csv")
GO_term_red %>%
  kbl() %>%
  kable_styling(bootstrap_options = "striped", full_width = F, position = "left") %>%
  kable_paper() %>%
...
```
##### Module 7
```{r GO-terms module number 7, message=FALSE, warning=FALSE}
GO_term_black <- read.csv("/Users/nuriteliash/Documents/GitHub/varroa-virus-networks/results/GO_terms_modules/GO_term_black.csv")
GO_term_black %>%
  kbl() %>%
  kable_styling(bootstrap_options = "striped", full_width = F, position = "left") %>%
  kable_paper() %>%
...
```
##### Module 9
```{r GO-terms module number 9, message=FALSE, warning=FALSE}
GO_term_magenta <- read.csv("/Users/nuriteliash/Documents/GitHub/varroa-virus-networks/results/GO_terms_modules/GO_term_magenta.csv")
GO_term_magenta %>%
  kbl() %>%
  kable_styling(bootstrap_options = "striped", full_width = F, position = "left") %>%
  kable_paper() %>%
...
```
##### Module 12
```{r GO-terms module number 13, message=FALSE, warning=FALSE}
GO_term_tan <- read.csv("/Users/nuriteliash/Documents/GitHub/varroa-virus-networks/results/GO_terms_modules/GO_terms_modules/GO_term_tan.csv")
GO_term_tan %>%
  kbl() %>%
  kable_styling(bootstrap_options = "striped", full_width = F, position = "left") %>%
  kable_paper() %>%
...
## Figure 3c. Correlating virus interaction and virus-varroa interactions
...
```{r Mantel-test for correlation between two distance-matrices, fig.cap = "Figure 3c. Intra-viral interactions can predict virus - vector interactions.  Correlation model between viral load correlations (fig 3a), and the distance-matrix of the module-virus correlations (fig 3b). For analysis in figure c, Mantel test for correlation between two matrices was conducted using 1,000 permutations." , message=FALSE, warning=FALSE}

#plot the correlation
#sizeGrWindow(9, 5)
verboseScatterplot(x = corModulTrait_66, y = virusAbundCor_66, main = "Correlation between Fig3a, \n virus-virus interaction; and Fig 3b, \n varroa-virus interaction", xlab = "Correlation of viral interaction \nwith varroa modules", ylab = "Correlation of viral abundance", abline = T, abline.color = "black", bg = "black", cex.lab = 1.2, cex.main = 1, cex.axis = 1)
```
In figure 3c, X-axis: how virus interacts with varroa expression, and Y-axis: the correlation of viral abundance across samples. 

We found a significant positive correlation between the two distance matrices (Mantel-test for correlation between two distance-matrices (Mantel 1967), Mantel statistic r = 0.604, p = 0.001) (Fig 3c). Meaning, given viruses’ interaction we can predict how these viruses will interact with the vector’s module.