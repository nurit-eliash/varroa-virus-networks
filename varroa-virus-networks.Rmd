...
# this table will be used for all further analysis
```
...
```
We therefore decided to remove the five outlier libraries from the data frame, and save for subsequent analyses 
...
maybe remove this chunk, as im not referring to these plots in the MS - בכל מקרה , זה כרגע לא עובד כאשר אני מנסה לסרוג את זה:)
```{r viral abundance -  remove???, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
prepare the "viralTraits" table of virus variables you wish to correlate to varroa modules
...
# For correlating varroa modules to viral load, we gonna use the table of 15 viruses saved in chunk "viral correlation", in section "viral correlation matrix", and correlated these to the varroa modules eigengenes. 
