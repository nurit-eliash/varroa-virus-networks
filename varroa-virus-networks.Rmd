...
In this study we investigated virus-virus and vector-virus interactions by two approaches: (1) meta-transciptomic and (2) gene-silencing experiments. 
...
In this R markdown document we present the meta-transcriptomic analysis step by step in a way that can be reproduced using the input files available on the GitHub webpage (_____).
The following chunks are ordered as the sections in the Materials and Methods of the manuscript "Vector-virus interactions in multi-viral infection", DOI:___
 ...
 *  We included only whole mites (not partial organs)
 
### Reads mapping and transcripts quantification
The reads were mapped to both available varroa genome (Vdes_3.0, accession number: GCF_002443255 (Techer et al. 2019)), and to the genomes of 23 selected viruses (table 1). The alignment and estimation of transcript and virus abundances were done using Kallisto (Bray et al. 2016) (version 0.46.1 with default options). The abundances were outputted in transcripts per million (TPM) units.

### Load libraries, data and RNAseq library filtration
Load libraries
...
### Creat the initial data frame 
The mapped libraries were used as input to make a data frame where each column is a SRR library, and each row is an isoform. The cells contain the isoform TPM.  
The output data frame contains 35,669 isoforms of 71 varroa libraries. the first 35,641 rows belong to the varroa genes, and the last 28 rows are reads that matched to varroa/bee viruses. 

```{r dataframe, echo=TRUE, message=FALSE, warning=FALSE}
...
# saveRDS(df_71, file = "/Users/nuriteliash/Documents/GitHub/varroa-virus-networks/data/kallisto_71.rds")
```

### Join isoforms into genes
```{r Join isoforms into genes,  echo=TRUE, message=FALSE, warning=FALSE}
...
# when we remove them and repeat the PCA, the library scatter more homogeneously.
```
...
### Remove outlier libraries and save data frame of varroa genes to WGCNA
...
# saveRDS(df, file = "/Users/nuriteliash/Documents/GitHub/varroa-virus-networks/data/kallisto.rds")
...
for_modules <- final_gene_tpm_71 %>% 
  ...
  transposeBigData() 
# saveRDS(for_modules, file = "/Users/nuriteliash/Documents/GitHub/varroa-virus-networks/data/for_modules.rds")
```
 

## Viral abundance analysis
AIM: describing viruses' abundance in the different varroa SRA libraries
first make the suitable data frame containing viruses' load in each library
```{r Creat data frame of virus load, message=FALSE, warning=FALSE}
...
```{r Choosing the soft-thresholding power, fig.cap = "Figure S2.  Network construction using 10,247 genes of 66 SRA varroa libraries.  a. picking soft threshold.", message=FALSE, warning=FALSE}
#readRDS(file = "/Users/nuriteliash/OneDrive - OIST/Repos/Archive/varroa-virus-networks-Local_OLD/Varroa-modules/for_modules.rds")
#modules <- log10(final_gene_tpm_66)
...
```{r Constructing the gene network, fig.cap = "Figure S2.  Network construction using 10,247 genes of 66 SRA varroa libraries.  a. picking soft threshold.", message=FALSE, warning=FALSE}
...
```{r Quantifying module–trait associations, fig.cap = "Figure 2. Intra-viral interactions can predict virus - vector interactions. b. Correlation between viruses’ loads and varroa modules (eigengenes).  Viruses and modules are ordered according to hierarchical clustering; P-values of  the Pearson coefficient are adjusted according to FDR-correction; correlation significance marked by (*) 0.1 < Padjust < 0.01; (**) Padjust < 0.01. For analysis in figure c, Mantel test for correlation between two matrices was conducted using 1,000 permutations. All viruses’ loads are transformed by log10 of the (TPM), Transcripts Per Million.", message=FALSE, warning=FALSE}
...
should i show Intramodular connectivity, if im not taling about it in the MS?
...
allGenes <- data.frame(
  ...
  dplyr::select(c(module, IntraModCon, MMpurple, padj.MMpurple, MMpink, padj.MMpink, MMgreen, padj.MMgreen, GS.DWVa, pdj.GS.DWVa, GS.VDV2, pdj.GS.VDV2, GS.ARV_2, pdj.GS.ARV_2)) %>%
...
genePurple = data.frame(
  ...
  MMPadjust_66,
  IntraModCon = Alldegrees1_66$kWithin) %>%
  dplyr::select(c(module, IntraModCon, MMpurple, padj.MMpurple, GS.DWVa, pdj.GS.DWVa, GS.VDV2, pdj.GS.VDV2, GS.ARV_2, pdj.GS.ARV_2)) %>%
  dplyr::filter(module == "purple") %>%
  rownames_to_column("genes")

write_csv(genePurple, "/Users/nuriteliash/Documents/GitHub/varroa-virus-networks/results/genePurple.csv")

### filtering for important genes in the module ###
# module membership filtration:
# MM > 0.6
# padj.MM < E-05
...
מה ההבדל בין זה לבין מה שחישבתי קודם 
ובמה מהם השתמשתי בשביל למצוא את הה
hubgenes>???
```{r Calculating Module membership for each gene, message=FALSE, warning=FALSE}
...
```{r Mantel-test for correlation between two distance-matrices, fig.cap = "Figure 2. Intra-viral interactions can predict virus - vector interactions.  c. Correlation model between viral load correlations (fig 2a), and the distance-matrix of the module-virus correlations (fig 2b). For analysis in figure c, Mantel test for correlation between two matrices was conducted using 1,000 permutations." , message=FALSE, warning=FALSE}

# load the two matrices:
# the module–trait association matrix 
# load(file = "moduleTraitCor_66.RData"); 

# and the viral abundance correlogram
# load(file = "virusAbundCor_66.RData"); 
