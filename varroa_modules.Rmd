...
mantel.test: the correlation btw two matrices
```{r}
...
MMPvalue = as.data.frame(corPvalueStudent(as.matrix(geneModuleMembership_66), nSamples));
...
GSPvalue = as.data.frame(corPvalueStudent(as.matrix(geneTraitSignificance_66), nSamples));
...
names(GSPvalue) = paste("p.GS.", names(b_viruses_load), sep="");

#save data:
save(geneTraitSignificance_66,geneModuleMembership_66, file = "/Users/nuriteliash/OneDrive - OIST/Repos/varroa-virus-networks-Local/results/geneTraitANDgeneMM_66.RData")
```
...
#### 
```{r HubGenes based on Module membership (MM) and virus-gene interaction, eval=FALSE, include=FALSE}
