...
#### WGCNA 3.a Quantifying module–trait associations (ModuleColors and ModuleLables)
```{r eval=F, echo=T}
lnames <- load(file = "/Users/nuriteliash/OneDrive - OIST/Repos/varroa-virus-networks-Local/results/varroa_virus-01-dataInput.RData"); 
lnames 
...
##########
# Recalculate MEs with module numbers 
MEnumbers0 = moduleEigengenes(for_modules, as.character(moduleLabels))$eigengenes
MEnumbers = orderMEs(MEnumbers0)
...
labeledHeatmap(Matrix = moduleTraitCor_66,
               xLabels = names(b_viruses_load),
               yLabels = names(MEnumbers),
               ySymbols = names(MEnumbers),
               colorLabels = FALSE,
               colors = hcl.colors(n = 50, palette = "RdBu", alpha = NULL, rev = FALSE, fixup = TRUE),
               invertColors = TRUE,
               textMatrix = textMatrix,
...
# or plot it in a "clean" mode, with module numbers:
sizeGrWindow(10,6)
# Will display correlations and their p-values
textMatrix = paste(signif(moduleTraitCor_66, 2), "\n(",
                   signif(moduleTraitPadjust_66, 1), ")", sep = "");
...
verboseScatterplot(x = virusAbundCor_66, y = corModulTrait_66, main = "Viral abundance predicts varroa-virus interaction", xlab = "Viruses interaction", ylab = "Varroa-virus interaction")

verboseScatterplot(x = corModulTrait_66, y = virusAbundCor_66, main = "Viral abundance predicts varroa-virus interaction", xlab = "Correlation of viral interaction with varroa modules", ylab = "Correlation of viral abundances")
```
...
Magenta <- dat %>%
  ...
  pivot_longer(cols = c(VDV4, VDV2),
                           names_to = "virus", values_to = "load")
Red <- dat %>%
  dplyr::select(c("library","MEred", "VDV4", "VDV2")) %>%
  pivot_longer(cols = c(VDV4, VDV2),
                           names_to = "virus", values_to = "load")

# and plot the eigengenes against the viral load
pMag <- ggplot(Magenta,aes(x = MEmagenta, y = load, col = virus, group = virus)) +
  geom_point() + 
